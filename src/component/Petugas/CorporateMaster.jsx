...
export default function CorporateMaster() {
  ...
  const [corporate, setCorporate] = React.useState({ name: "" });
  ...
  // Fungsi Helper untuk Modal (Clean Code)
  const getModal = () => {
    const modalEl = document.getElementById("modalNamaMaster");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEdit
        ? `${import.meta.env.VITE_API_URL}/api/corporates/${currentUuid}`
        : `${import.meta.env.VITE_API_URL}/api/corporates`;
      ...
      getCorporate();
      if (isEdit) {
        showAlert("info", "Diperbarui!", `Data "${corporate.name}" berhasil diperbarui.`);
      } else {
        showAlert("success", "Berhasil Ditambahkan!", `Data "${corporate.name}" berhasil disimpan.`);
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/corporates/${deleteTarget.uuid}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
    ...
    <AdminLayout>
      ...
      <div className="page-wrapper px-4 py-4">
        ...
        <div className="row mb-4 align-items-center">
          <div className="col">
            <h4 className="fw-bold mb-0 text-dark">Manajemen Data Master</h4>
            <p className="text-muted small mb-0">
              Kelola daftar arsip dan kategori master data Anda.
            </p>
          </div>
          <div className="col-auto">
            ...
            >
              <i className="bx bx-plus-circle"></i> Tambah Data Baru
            </button>
          </div>
        </div>
            ...
            >
              <div className="modal-header border-0 pt-4 px-4">
                <h5 className="fw-bold">
                  {isEdit ? "Perbarui Data" : "Tambah Data Baru"}
                </h5>
                <button
                  type="button"
                  className="btn-close"
                ...
                <div className="modal-body p-4">
                  <div className="text-center mb-4">
                    <div className="bg-light d-inline-block p-4 rounded-circle mb-3">
                      <i
                        className={`bx ${isEdit ? "bx-edit" : "bx-file"} fs-1 text-primary`}
                      ></i>
                    </div>
                    <p className="text-muted small">
                      Pastikan nama Instansi yang Anda masukkan sudah sesuai
                    </p>
                  </div>
                  <div className="mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) => setCorporate({ name: e.target.value })}
