...
export default function LantaiMaster() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      refreshData();
      if (isEdit) {
        showAlert("info", "Diperbarui!", `Data "${floor.name}" berhasil diperbarui.`);
      } else {
        showAlert("success", "Berhasil Ditambahkan!", `Data "${floor.name}" berhasil disimpan.`);
      }
    } catch (error) {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState(null);
        ...
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_URL}/api/floors/${deleteTarget.uuid}`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
