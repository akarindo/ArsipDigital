...
export default function RuangMaster() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      refreshData();
      if (isEdit) {
        showAlert("info", "Diperbarui!", `Data "${room.name}" berhasil diperbarui.`);
      } else {
        showAlert("success", "Berhasil Ditambahkan!", `Data "${room.name}" berhasil disimpan.`);
      }
    } catch (error) {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState(null);
        ...
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_URL}/api/rooms/${deleteTarget.uuid}`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
