...
export default function KategoriMaster() {
  ...
  const getModal = () => {
    const modalEl = document.getElementById("tambahKategoriMaster");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      refreshData();
      if (isEdit) {
        showAlert("info", "Diperbarui!", `Data "${category.name}" berhasil diperbarui.`);
      } else {
        showAlert("success", "Berhasil Ditambahkan!", `Data "${category.name}" berhasil disimpan.`);
      }
    } catch (error) {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState(null);
        ...
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_URL}/api/kategori/${deleteTarget.uuid}`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
    ...
    <AdminLayout>
      ...
      <div className="page-wrapper px-4 py-4">
        ...
        <div className="row mb-4 align-items-center">
          <div className="col">
            <h4 className="fw-bold mb-0 text-dark">Data Master Kategori</h4>
            <p className="text-muted small mb-0">
              Pengelompokan arsip berdasarkan label kategori utama.
            </p>
          </div>
          <div className="col-auto">
            ...
            >
              <i className="bx bx-plus-circle"></i> Tambah Kategori
            </button>
          </div>
        </div>
                  ...
                  >
                    <div className="card-body d-flex align-items-center justify-content-between p-3">
                      <div className="d-flex align-items-center">
                        ...
                        >
                          <i className="bx bx-tag-alt text-warning fs-4"></i>
                        </div>
                        <div>
                ...
                <div className="modal-body p-4">
                  <div className="text-center mb-4">
                    <div className="bg-light d-inline-block p-4 rounded-circle mb-3">
                      <i
                        className={`bx ${isEdit ? "bx-edit" : "bx-tag"} fs-1 text-warning`}
                      ></i>
                    </div>
                    <p className="text-muted small">
                      Label kategori membantu dalam memfilter dokumen dengan
                      cepat
                    </p>
                  </div>
                  <div className="mb-3">
