...
export default function NamaMaster() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      refreshData();
      if (isEdit) {
        showAlert("info", "Diperbarui!", `Data "${name.name}" berhasil diperbarui.`);
      } else {
        showAlert("success", "Berhasil Ditambahkan!", `Data "${name.name}" berhasil disimpan.`);
      }
    } catch (error) {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState(null);
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/names/${deleteTarget.uuid}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
